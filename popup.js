...
async function doSync() {
    const destId = document.querySelector('input[name="dest"]:checked').value;

    if (!chrome.bookmarks) {
        setStatus('error', 'Missing bookmarks permission — reload the extension in chrome://extensions/');
        return;
    }
    ...
    try {
        ...
        setStatus('syncing', 'Fetching collections…');
        const allCols = await fetchAllCollections();
        const cols = allCols.filter(c => c._id !== -99); // skip trash
        ...
        // ── Phase 3: build Chrome bookmark folder tree (BFS) ──
        setStatus('syncing', 'Building folder structure…');

        const folderMap = {}; // raindropColId → chromeBookmarkId
        const depthMap  = {}; // raindropColId → nesting depth

        let queue = cols
            .filter(c => !c.parent || !c.parent.$id)
            .map(c => ({ col: c, parentId: rootFolder.id, depth: 0 }));

        while (queue.length > 0) {
            const nextQueue = [];
            await Promise.all(queue.map(async ({ col, parentId, depth }) => {
                const folder = await chrome.bookmarks.create({ parentId, title: col.title });
                folderMap[col._id] = folder.id;
                depthMap[col._id]  = depth;
                cols
                    .filter(c => c.parent && c.parent.$id === col._id)
                    .forEach(child => nextQueue.push({ col: child, parentId: folder.id, depth: depth + 1 }));
            }));
            queue = nextQueue;
        }

        // ── Phase 4: fetch raindrops and populate folders ──
        const colsWithDrops = cols.filter(c => c.count > 0 && folderMap[c._id] !== undefined);
        let done = 0;
        let totalSynced = 0;
        let totalErrors = 0;
        const syncLog = [];
        ...
        await Promise.all(colsWithDrops.map(async col => {
            ...
            try {
                ...
                totalSynced += entry.synced;
            } catch (e) {
                entry.error = e.message;
                totalErrors++;
                console.error(`Failed collection "${col.title}":`, e);
            } finally {
                syncLog.push(entry);
                done++;
                setStatus('syncing', `Syncing bookmarks… (${done} / ${colsWithDrops.length})`);
            }
        }));

        // ── Store results ──
        const totalExpected = colsWithDrops.reduce((n, c) => n + c.count, 0);
        await chrome.storage.local.set({
            lastSyncTime:     Date.now(),
            lastSyncCols:     cols.length,
            lastSyncExpected: totalExpected,
            lastSyncActual:   totalSynced,
            lastSyncErrors:   totalErrors,
            syncLog
        });
        ...
        if (totalErrors > 0) {
            setStatus('error',
                `Synced ${cols.length} collections · ${totalSynced}/${totalExpected} bookmarks · ${totalErrors} error(s)`
            );
        } else {
            setStatus('success',
                `Synced ${cols.length} collections · ${totalSynced} bookmarks`
            );
        }
        show($('report-link'));
        ...
        setStatus('error', `Sync failed: ${e?.message || e}`);
    } finally {
        setSyncing(false);
    }
}
...
function setSyncing(active) {
    $('sync-btn').disabled = active;
}

// ── Views ─────────────────────────────────────────────────────────────────────
function showSetupView() { show($('setup-view')); hide($('main-view')); }
function showMainView()  { hide($('setup-view')); show($('main-view')); }
...
function initSetupView() {
    ...
    $('token-input').addEventListener('keydown', e => { if (e.key === 'Enter') connectToken(); });
}
...
async function restoreLastSyncStatus() {
    const s = await chrome.storage.local.get(
        ['lastSyncTime', 'lastSyncCols', 'lastSyncExpected', 'lastSyncActual', 'lastSyncErrors', 'rdDest']
    );
    if (s.rdDest) {
    ...
    if (s.lastSyncTime) {
        const mismatch = s.lastSyncActual < s.lastSyncExpected;
        const hasErr   = s.lastSyncErrors > 0;
        const type     = (mismatch || hasErr) ? 'error' : 'success';
        setStatus(type,
            `Last synced ${timeAgo(s.lastSyncTime)} · ${s.lastSyncCols} collections · ` +
            `${s.lastSyncActual}/${s.lastSyncExpected} bookmarks`
        );
        show($('report-link'));
    } else {
...
function initMainView() {
    ...
    document.querySelectorAll('input[name="dest"]').forEach(r => {
        r.addEventListener('change', () => chrome.storage.local.set({ rdDest: r.value }));
    });
    ...
    $('disconnect-btn').addEventListener('click', async () => {
        await chrome.storage.local.remove(
            ['rdToken', 'lastSyncTime', 'lastSyncCols', 'lastSyncExpected',
             'lastSyncActual', 'lastSyncErrors', 'syncLog', 'rdDest']
        );
        token = null;
        showSetupView();
        initSetupView();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!token) { showSetupView(); initSetupView(); return; }
    showMainView();
    initMainView();
    restoreLastSyncStatus();
});
